export const BACKEND_URL = 'https://api.hrodvitnir.pw/';

export const BACKEND_AVATAR_PATH = '/file/';

export const NOTIFICATION_SHOW_TIME = 3000; // ms

export const PAGES_MAX_VALUE = 32500;

export const BOOK_RELOAD_TIMEOUT_SECONDS = 43200;

export const BOOK_RELOAD_DELAY_SECONDS = 10;

export const VERSION = 'alpha-0.1.12';

export const HTTP_TRYOUTS_COUNT = 3;

export const HTTP_REQUEST_TIMEOUT_MS = 12000;