...
export default {
  ...
  methods: {
    async editGenre(newName) {
      try {
        await this.$store.dispatch(GENRE_RENAME_ACTION, {
          oldName: this.name,
          newName,
        });
        ...
        this.$store.dispatch(
          NOTIFICATION_SUCCESS_ACTION,
          this.$t("book.lists.genre.rename.success")
        );
      } catch (e) {
        if (e == NETWORK_ERROR) {
          this.name = newName;
          ...
          this.$store.dispatch(
            NOTIFICATION_DANGER_ACTION,
            this.$t("book.lists.genre.rename.error")
          );
        }
      }
    },
