...
export const getters = {
    ...
    [BOOKS_IN_PROGRESS_GETTER]: (state, getters) => u(getters.books).where(item => item.status === IN_PROGRESS_STATUS).toArray(),
    [BOOKS_TO_READ_GETTER]: (state, getters) => u(getters.books).where(item => item.status === TO_READ_STATUS).toArray(),
    [BOOKS_DONE_GETTER]: (state, getters) => u(getters.books).where(item => item.status === DONE_STATUS).toArray(),
    [BOOKS_GENRES_COUNT_GETTER]: (state, getters) => u(getters[BOOKS_DONE_GETTER])
